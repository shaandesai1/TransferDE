...
class ODEFunc(nn.Module):
    ...
    def get_key(self,y,t):
        key = self.key_net(y)
        self.keys[t] = key
        return key

    def get_query(self, y, t):
        query = self.query_net(y)
        self.queries[t] = query
        return query

    def get_value(self, y, t):
        value = self.value_net(y)
        self.values[t] = value
        return value

    def get_C(self,query):
        alpha_up = 0.
        alpha_down = 0.
        for i,key in self.keys.items():

            qk = torch.exp(torch.diagonal(torch.matmul(query.squeeze(1), key.squeeze(1).T), 0).reshape(-1, 1, 1))
            qk_reshape = qk.squeeze().repeat_interleave(2).reshape(-1, 1, 2)
            alpha_up += qk_reshape*self.values[i]
            alpha_down += qk_reshape
    ...
    def forward(self, t, inputs):
        ...
        # ts = torch.ones_like(y)*t
        _ = self.get_key(y, t)
        query = self.get_query(y, t)
        _ = self.get_value(y, t)

        c = self.get_C(query)

        # z_hidden = self.z_net(torch.cat([y,c],-1))
        z_hidden = self.z_net(c)
        cdot = torch.zeros_like(z_hidden)
        Adot = torch.zeros_like(z_hidden)
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                ...
                pred_y = odeint(func, main_input_test, t,method='euler')
                N = int(len(pred_y[1]) // 3)
