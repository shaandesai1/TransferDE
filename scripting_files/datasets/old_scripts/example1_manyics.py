...
def get_batch():
    ...
    batch_y0 = true_y[s][:args.number_ics].reshape(args.number_ics,2)  # (M, D)
    batch_t = t[:args.batch_time]  # (T)
    batch_y = torch.stack([true_y[s + i][:args.number_ics].reshape(args.number_ics,2) for i in range(args.batch_time)], dim=0)  # (T, M, D)
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        integ = odeint(func, batch_y0, batch_t, method='euler').to(device)
        pred_y =  integ
        ...
        sdothat = torch.stack([xd.reshape(-1, args.number_ics), pd.reshape(-1, args.number_ics)], 2)
        print(sdothat.shape)
