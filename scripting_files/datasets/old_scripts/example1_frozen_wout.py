...
def get_wout(s, sd, y0):
    y0 = torch.stack([y0 for i in range(len(s))]).reshape(-1, 1, 2)

    # A1 = torch.tensor([[1., 0.], [0., 1.]])
    # A0 = torch.tensor([[0., -1.], [1., 0.]])

    dh_i = sd + s


    # print(sd.shape,s.shape)
    # DH = torch.mm(sd, A1) + torch.mm(s, A0)
    # D0 = torch.mm(y0, A0)
    # lambda_0 = 0.1
    # W0 = -torch.mm(torch.mm(torch.inverse(torch.mm(DH.T, DH) + lambda_0), DH.T), D0)
    # return W0
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        integdot = torch.cat([integdot, bias2], 2)

        gt = (1. - torch.exp(-batch_t)).repeat_interleave(NDIMZ + 1).reshape(-1, 1, NDIMZ + 1)
        gtd = (torch.exp(-batch_t)).repeat_interleave(NDIMZ + 1).reshape(-1, 1, NDIMZ + 1)

        s = gt * integ
        sd = gtd * integ + gt * integdot

        wout = get_wout(s, sd, batch_y0)

        pred_y = batch_y0.reshape(1, 1, 2) + torch.mm(s, wout)
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                gt = (1. - torch.exp(-t)).repeat_interleave(1 + NDIMZ).reshape(-1, 1, NDIMZ + 1)
                zinit = torch.zeros(NDIMZ).reshape(1, NDIMZ)
                ...
                bias2 = torch.zeros((len(t), 1, 1))
                integdot = torch.cat([integdot, bias2], 2)

                gt = (1. - torch.exp(-t)).repeat_interleave(NDIMZ + 1).reshape(-1, 1, NDIMZ + 1)
                gtd = (torch.exp(-t)).repeat_interleave(NDIMZ + 1).reshape(-1, 1, NDIMZ + 1)
                ...
                pred_y = true_y0.reshape(1, 1, 2) + torch.mm(s, wout)
