...
parser.add_argument('--number_ics', type=int, default=5)
...
xinits = torch.distributions.Uniform(1.4, 2.5).sample([20,1])
...
print(f'true Y{true_y.shape}')


def get_batch():
    vv = torch.from_numpy(np.random.choice([0,1,2,3,4,5],1,replace=False))
    s = torch.from_numpy(np.random.choice(np.arange(args.data_size - args.batch_time, dtype=np.int64), args.batch_size, replace=False))
    print(true_y.shape)
    batch_y0 = true_y[s,vv,:].reshape(args.batch_size,2)  # (M, D)
    batch_t = t[:args.batch_time]  # (T)
    batch_y = torch.stack([true_y[s + i,vv,:].reshape(args.batch_size,2) for i in range(args.batch_time)], dim=0)  # (T, M, D)
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        integ = odeint(func, batch_y0, batch_t, method='rk4').to(device)
        pred_y =  integ
        print(pred_y.shape)
        pred_ydot = torch.stack([func(batch_t[i], integ[i]) for i in range(len(integ))])
        x, p = pred_y[:, :, 0], pred_y[:, :, 1]
        xd = p
        pd = -x - x ** 3
        # print(pred_y.shape,pred_ydot.shape)
        sdothat = torch.stack([xd.reshape(-1, args.batch_size), pd.reshape(-1, args.batch_size)], 2)
        # print(sdothat.shape)
        loss = torch.mean((pred_ydot - sdothat) ** 2)
        loss.backward()
        optimizer.step()
