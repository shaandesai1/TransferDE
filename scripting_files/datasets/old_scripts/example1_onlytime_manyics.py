...
# print(newics)
# true_y0 = torch.tensor([[2., 0.5]]).to(device)
    ...
    if args.viz:
        ...
        for i in range(1):
            ax_traj.plot(t.cpu().numpy(), true_y.cpu().numpy()[:, i, 0], t.cpu().numpy(), true_y.cpu().numpy()[:, i, 1],
                         'g-')
            ax_traj.plot(t.cpu().numpy(), pred_y.cpu().numpy()[:, i, 0], '--', t.cpu().numpy(),pred_y.cpu().numpy()[:, i, 1], 'b--')
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        # print(batch_t.shape)
        zinit = Lambda()(0,batch_y0).reshape(-1,2)#torch.tensor([[0.5,-10.]]).reshape(-1,2)#torch.ones_like(batch_y0)
        print(batch_y0,zinit)
        integ = odeint(func, zinit, batch_t,method='euler').to(device)
        gt = (1. - torch.exp(-batch_t)).repeat_interleave(2).reshape(-1,1,2)
        # print(gt)
        gtd = (torch.exp(-batch_t)).repeat_interleave(2).reshape(-1,1,2)
        # print(integ.shape,batch_t.shape)
        pred_y = batch_y0.reshape(1,1,2) + gt*integ
        # print(func(batch_t[0],pred_y[0]))
        pred_ydot = gt*torch.stack([func(batch_t[i], integ[i]) for i in range(len(integ))]) + gtd*integ
        # print(pred_y.shape)
        # print(pred_y.shape)
        # print(pred_y[0],pred_ydot[0])
        x, p = pred_y[:,:, 0], pred_y[:,:,1]
        xd = p
        pd = -x - x ** 3
        sdothat =  torch.cat([xd.reshape(-1, 1), pd.reshape(-1, 1)], 1)
        # print(pred_ydot.shape,sdothat.shape)
        loss = torch.mean((pred_ydot.squeeze()-sdothat)**2)
        loss.backward()
        optimizer.step()
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                gt = (1. - torch.exp(-t)).repeat_interleave(2).reshape(-1, 1, 2)

                zinit = true_ydot#torch.tensor([[0.5,-2-2**3]]).reshape(-1,2)
                pred_y = torch.stack([true_y0 for i in range(args.batch_time)]) + gt*odeint(func, zinit, t,method='euler')
