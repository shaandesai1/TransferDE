...
true_y0 = torch.tensor([[2., 0., 12.]]).to(device)
...
class Lambda(nn.Module):
    ...
    def forward(self, t, y):
        ...
        yd = x * (rho - z) - y
        zd = x * y - beta * z
        return torch.cat([xd.reshape(-1, 1), yd.reshape(-1, 1), zd.reshape(-1, 1)], 1)
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        # pred_y_ = (pred_y**3).reshape(-1,2)
        # print(pred_y.shape)
        # print(pred_y.shape)
        pred_x, pred_yy, pred_z = pred_y[:, :, 0], pred_y[:, :, 1], pred_y[:, :, 2]

        sigma = 10
        rho = 28
        beta = 8. / 3

        xd = sigma * (pred_yy - pred_x)
        yd = pred_x * (rho - pred_z) - pred_yy
        zd = pred_x * pred_yy - beta * pred_z

        loss = torch.mean(
            (pred_ydot.reshape(-1, 3) - torch.cat([xd.reshape(-1, 1), yd.reshape(-1, 1), zd.reshape(-1, 1)], 1)) ** 2)
