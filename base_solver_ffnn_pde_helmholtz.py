...
def get_rho(t,x,x01,y01,x02,y02):
    ...
    # c = torch.sin(theta) ** 2 / (2 * sigma_X ** 2) + torch.cos(theta) ** 2 / (2 * sigma_Y ** 2);
    #
    # Z1 = A * torch.exp(-(a * (X - (x0-2.5)) ** 2 + 2 * b * (X - (x0-2.5)) * (Y - y0) + c * (Y - y0) ** 2));
    # # Z2 = A * torch.exp(-(a * (X - (x0+2.5)) ** 2 + 2 * b * (X - (x0+2.5)) * (Y - y0) + c * (Y - y0) ** 2));
    #
    # return Z1 #+ Z2
    # plt.contour(X, Y, Z);
...
if __name__ == '__main__':
    ...
    func = ODEFunc(hidden_dim=NDIMZ,output_dim=5)
    ...
    if not args.evaluate_only:
        ...
        for itr in range(1, args.niters + 1):
            ...
            rhos = torch.cat([rho1,rho2,rho3,rho4,rho5],1).reshape(-1,5)
            ...
            if itr % args.test_freq == 0:
                ...
                if current_residual < best_residual:
                    torch.save(func.state_dict(), 'func_ffnn_helm_1')
    ...
    func.load_state_dict(torch.load('func_ffnn_helm_1'))
    func.eval()
    ...
    with torch.no_grad():
        out_pred = (H@WOUT)
        # print((out_pred))
        loss_test =Htt@WOUT + Hxx@WOUT + kval*out_pred-rho(grid_tt.reshape(-1,1),grid_xx.reshape(-1,1))
