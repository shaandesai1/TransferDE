...
parser.add_argument('--niters_wout', type=int, default=2000)
...
class diffeq(nn.Module):
    ...
    def forward(self, t, y):
        y = y[:, 0]
        yd = (-self.a0(t)*y -0.5*y**2+ self.f(t)) / self.a1(t)
        return yd
...
class ODEFunc(nn.Module):
    ...
    def forward(self, t, y):
        # tt = torch.ones(len(y))*t
        # print(tt.shape,y.shape)
...
if __name__ == '__main__':
    ...
    zinit = torch.randn(NDIMZ).reshape(1, NDIMZ)
    ...
    if args.evaluate_only is False:
        for itr in range(1, args.niters + 1):
            ...
            lst = (a1(t).reshape(-1,1))*pred_ydot +0.5*(pred_y**2).reshape(-1,1)+ (a0(t).reshape(-1,1))*pred_y - f(t).reshape(-1,1)
    ...
    for ic in ics:
        ...
        optimizer = optim.SGD(wout_gen.parameters(), lr=1e-3, weight_decay=1e-3)
        ...
        for itr in range(1, args.niters_wout + 1):
            ...
            # print(wout)
            pred_y = ic.reshape(1, 1) + torch.mm(s, wout)
            pred_ydot = torch.mm(sd, wout)
            lst = (a1(t).reshape(-1, 1)) * pred_ydot + 0.5 * (pred_y ** 2).reshape(-1, 1) + (
                a0(t).reshape(-1, 1)) * pred_y - f(t).reshape(-1, 1)
            loss = torch.mean((lst) ** 2)
            # print(loss)
            print(loss)
            loss.backward()
        ...
        with torch.no_grad():
            ...
            pred_ydot = torch.mm(sd,wout)
            rmsr += ((a1(t).reshape(-1,1))*pred_ydot + 0.5*(pred_y**2).reshape(-1,1)+ (a0(t).reshape(-1,1))*pred_y - f(t).reshape(-1,1))**2
            pred_y = pred_y.reshape(-1, 1, 1)
