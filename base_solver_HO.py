...
parser.add_argument('--niters_wout', type=int, default=5000)
...
if __name__ == '__main__':
    ...
    if args.wout == 'analytic':
        ...
        wout_gen = Transformer_Learned(NDIMZ,1,true_y0.shape[1])
    ...
    if args.wout  == 'analytic':
        ...
        optimizer = optim.Adam([
            ...
            {'params': wout_gen.parameters(), 'lr': 1e-2,'weight_decay': 1e-3}
        ])



    param = Parametrization(args.paramg)
    zinit = torch.zeros(1,NDIMZ)#torch.randn(NDIMZ).reshape(1, NDIMZ)
    ...
    if args.evaluate_only is False:
        for itr in range(1, args.niters + 1):
            ...
            lst = ((pred_qd-pred_p)**2) + ((pred_pd + pred_q)**2) + (get_ham(pred_q,pred_p).reshape(-1,1) - get_ham(true_y0[0,0],true_y0[0,1]).reshape(1,1))**2#+ energy term
            loss = torch.mean(lst)
            print(loss.item())
            # print(f'sd:{sd[0]}')
            # print(f'wout:{wout[0]}')
        ...
        torch.save(wout_gen.state_dict(), 'wout_dict_wout')

    ii = 0
    func = ODEFunc(NDIMZ)
    func.load_state_dict(torch.load('func_dict_wout'))
    func.eval()
    # freeze all weights
    ...
    s, sd = compute_s_sdot(func, zinit, t, param)
    q_ics = torch.linspace(0.5, 2., 20)
    p_ics = torch.linspace(0.5, 2., 20)
    ...
    for ic in ics:
        ...
        wout_gen.load_state_dict(torch.load('wout_dict_wout'))
        optimizer = optim.Adam(wout_gen.parameters(), lr=1e-3, weight_decay=1e-3)
        ...
        for itr in range(1, args.niters_wout + 1):
            ...
            pred_pd = torch.mm(sd, wout[1])
            lst = (pred_qd - pred_p) ** 2 + (pred_pd + pred_q + 0*pred_q ** 3) ** 2  # + energy term
            loss = torch.mean(lst)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            ...
            rmsr += (pred_qd - pred_p) ** 2 + (pred_pd + pred_q + 0.*pred_q ** 3) ** 2  # + energy term
