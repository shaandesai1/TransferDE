...
if __name__ == '__main__':
    ...
    if not args.evaluate_only:
        ...
        for itr in range(1, args.niters + 1):
            ...
            u_t0 = torch.mean((func(torch.zeros(len(x_evals)).reshape(-1,1),x_evals.reshape(-1,1)).ravel() - ic_t0.ravel())**2)
        ...
        torch.save(func.state_dict(), 'func_ffnn_pde')
    ...
    func.load_state_dict(torch.load('func_ffnn_pde'))
    func.eval()
    ...
    with torch.no_grad():
        ...
        u_true = 3*torch.sin(grid_xx) * torch.exp(-lambda_val*grid_tt)
