...
class ODEFunc(nn.Module):
    ...
    def __init__(self, number_dims):
        ...
        self.l1 = nn.Linear(self.number_dims+1,200,bias=True)
        self.l2 = nn.Linear(200,self.number_dims,bias=False)
        self.nonlin = SiLU()

        # self.upper = nn.Sequential(
        #     nn.Linear(self.number_dims+1, 200,bias = True),
        #     SiLU(),
        #     nn.Linear(200, self.number_dims,bias = False))

    def forward(self, t, y):

        inputs_ = torch.cat([y,t.reshape(-1,1)],1)
        x = self.l1(inputs_)
        x = self.nonlin(x)
        x = self.l2(x)
        return x

    def high_dim(self, t, y):
...
class Transformer_Learned(nn.Module):
    ...
    def __init__(self, hidden_dims=200, output_dims=2,num_systems=1):
        super(Transformer_Learned, self).__init__()
        self.linear = nn.Linear(200,2,bias=False)
...
def compute_s_sdot(func,zinit,batch_t,param):
    ...
    gt = param.get_g(t).repeat_interleave(NDIMZ+0 ).reshape(-1, NDIMZ+0)
    gtd = param.get_gdot(t).repeat_interleave(NDIMZ+0).reshape(-1, NDIMZ+0 )
...
if __name__ == '__main__':
    ...
    if args.evaluate_only is False:
        for itr in range(1, args.niters + 1):
            ...
            pred_q = true_y0[0,0].reshape(1, 1) + s[:,0].reshape(-1,1)
            pred_qd = sd[:,0].reshape(-1,1)
            pred_p = true_y0[0,1].reshape(1,1) + s[:,1].reshape(-1,1)
            pred_pd = sd[:,1].reshape(-1,1)
            ...
            if itr % args.test_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    visualize(true_y, pred_y, func, ii,s,sd,1)
                    ii += 1
    ...
    ii = 0
    # q_ics = torch.tensor([1.3])#torch.linspace(1., 1.5, 3)
    # p_ics = torch.tensor([1.])#torch.linspace(0.8, 1.2, 3)

    q_ics = torch.linspace(1., 2., 10)
    p_ics = torch.linspace(0.8, 2., 10)
    ...
    for ic in ics:
        ...
        func.eval()
        # freeze all weights
        for name, param_v in func.named_parameters():
            print(name)
            param_v.requires_grad = False

        with torch.no_grad():
            integ = odeint(func, zinit, t.ravel(), method=args.method_rc)
            integdot = torch.stack([func(t.ravel()[i], integ[i]) for i in range(len(integ))])

            integ = integ.reshape(-1, NDIMZ)
            integdot = integdot.reshape(-1, NDIMZ)

            # az = func.high_dim(t,integ)
            azd = func.high_dim(t, integ)

            # az_0 = torch.ones(200).reshape(-1,1)

            az = torch.zeros(len(azd),200)
            for i in range(1,len(azd)):
                az[i] = az[i-1] + args.dt*azd[i-1]
            # azs = []
            # azd = func.high_dim(t, integdot)

            gt = param.get_g(t).repeat_interleave(200 + 0).reshape(-1, 200 + 0)
            gtd = param.get_gdot(t).repeat_interleave(200 + 0).reshape(-1, 200 + 0)
        ...
        mdl = Transformer_Learned()

        # for name, param_v in func.named_parameters():
        #     if name == 'l2.weight':
        #         mdl.linear.weight = copy.deepcopy(param_v)
        #         mdl.linear.weight.requires_grad = True
        #     if name == 'l2.bias':
        #         mdl.linear.bias = copy.deepcopy(param_v)
        #         mdl.linear.bias.requires_grad = True
        ...
        for itr in range(1, args.niters_wout + 1):
            ...
            pred_q = ic[0].reshape(1, 1) + mdl(gz)[:,0].reshape(-1,1)#s[:,0].reshape(-1,1)
            pred_qd = mdl(gzd)[:,0].reshape(-1,1)
            pred_p = ic[1].reshape(1, 1) + mdl(gz)[:,1].reshape(-1,1)
            pred_pd = mdl(gzd)[:,1].reshape(-1,1)
